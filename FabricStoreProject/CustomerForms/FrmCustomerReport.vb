...
Public Class FrmCustomerReport
    ...
    Private Sub FrmCustomerReport_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged
    ...
    Private Sub Search()
        ...
        SQLQuery = "   Select ROW_NUMBER() OVER (ORDER BY (ID)) As ت, * From CustomersTable where  EndService IS NULL  "

        AppendConditions()

        SQLQuery &= " ORDER BY ID  OFFSET " & PageSize * (PageNum - 1) & " ROWS FETCH NEXT " & PageSize & " ROWS ONLY"

        SQLQuery &= " Select COUNT(CustomersTable.ID)  From  CustomersTable where  EndService IS NULL "
        AppendConditions()
    ...
    Private Sub DGVEmpReport_DoubleClick(sender As Object, e As EventArgs) Handles DGVReport.DoubleClick
        ...
        CustomerReportID = DGVReport.Item(0, CR).Value
        FrmCustomerContainr.LblInfo_Click(sender, e)
    End Sub
