...
Public Class FrmDeliveryCo
    ...
    Private Sub FrmDeliveryCo_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged
        If Me.Visible = True Then
            GetData()
            TxtName.Focus()
            If DeliveryReportID <> 0 Then
                CmbName.SelectedValue = DeliveryReportID
    ...
    Private Sub FrmDeliveryCo_Load(sender As Object, e As EventArgs) Handles Me.Load
        Me.Height = 680
        TxtName.Focus()
        DGVCompany.RowTemplate.Height = DGVCompany.Height - DGVCompany.ColumnHeadersHeight
    End Sub
    ...
    Private Sub BtnShow_Click(sender As Object, e As EventArgs) Handles BtnShow.Click
        ...
        If IsNothing(CmbName.SelectedValue) Then
            MsgTool("يرجى اختيار شركة من شريط البحث", 0)
    ...
    Private Sub GetData()
        ...
        SQLQuery = " Select ID,Name from DeliveryTable  where EndService IS NULL  "
    ...
    Private Sub DisplayData()
        ...
        SQLQuery = " Select * From DeliveryTable Where EndService IS NULL and ID = " & TailorID
    ...
    Private Sub FillDGV()
        ...
        For i = 0 To DSDisplay.Tables(0).Rows.Count - 1
            With DSDisplay.Tables(0).Rows(i)
                DGVCompany.Rows.Add()
                DGVCompany.Item(0, i).Value = .Item("ID")
                DGVCompany.Item(1, i).Value = .Item("Name")
                DGVCompany.Item(2, i).Value = .Item("Address")

                If Not .Item("Phone1").ToString = "" Then
                    DGVCompany.Item(3, i).Value = .Item("Phone1")
                End If

                If Not .Item("Phone2").ToString = "" Then
                    If DGVCompany.Item(3, i).Value.ToString = "" Then
                        DGVCompany.Item(3, i).Value = .Item("Phone2")

                    Else
                        DGVCompany.Item(3, i).Value = DGVCompany.Item(3, i).Value & " - " & .Item("Phone2")
                    End If
                End If
    ...
    Private Sub BtnDeleteBasic_Click(sender As Object, e As EventArgs) Handles BtnDeleteBasic.Click
        ...
        If MsgBox("هل تريد بالتأكيد الحذف ؟", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "تأكيد الحذف") = MsgBoxResult.Yes Then
            ...
            Dim sqlCon As New SQLConClass()
            Dim s As Integer = sqlCon.CMDExecute("DeleteDelivery", 1, param)
            If s = 1 Then
                MsgTool("تم الحذف بنجاح", 1)

            ElseIf s = 2 Then
                MsgTool("لا يمكن حذف الشركة ، الشركة لديها فواتير", 0)
            End If
    ...
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles BtnSave.Click
        ...
        DSSave = Sqlcon.CMDExecuteData("SaveDelivery", param)
    ...
    Private Sub DGVCompany_Click(sender As Object, e As EventArgs) Handles DGVCompany.Click
        If CheckDGVError(DGVCompany) = True Then Exit Sub
        TailorID = DGVCompany.CurrentRow.Index
        DisplayData()
    End Sub
