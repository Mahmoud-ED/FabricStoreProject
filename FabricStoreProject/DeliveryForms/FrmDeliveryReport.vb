...
Public Class FrmDeliveryReport
    ...
    Private FromSearch, FromLoad As Boolean

    Private Sub FrmDeliveryReport_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged
    ...
    Private Sub Search()
        ...
        SQLQuery = "   Select ROW_NUMBER() OVER (ORDER BY (ID)) As ت, * From DeliveryTable where  EndService IS NULL  "

        AppendConditions()

        SQLQuery &= " ORDER BY ID  OFFSET " & PageSize * (PageNum - 1) & " ROWS FETCH NEXT " & PageSize & " ROWS ONLY"

        SQLQuery &= " Select COUNT(DeliveryTable.ID)  From  DeliveryTable where  EndService IS NULL "
        AppendConditions()
    ...
    Private Sub FillDGV()
        ...
        For i = 0 To DSLoadSearch.Tables(0).Rows.Count - 1
            ...
            With DSLoadSearch.Tables(0).Rows(i)
                ...
                If Not .Item("Phone1").ToString = "" Then
                    DGVReport.Item(3, i).Value = .Item("Phone1")
                End If

                If Not .Item("Phone2").ToString = "" Then
                    If DGVReport.Item(3, i).Value.ToString = "" Then
    ...
    End Sub
    '------------------------------------CheckBox-----------------------------------------------------
    Private Sub ChkNameReport_CheckedChanged(sender As Object, e As EventArgs) Handles ChkNameReport.CheckedChanged
    ...
    Private Sub BtnPrintAll_Click(sender As Object, e As EventArgs) Handles BtnPrintAll.Click
        ...
        SQLQuery = "SELECT Name,Phone1,Phone2, Address FROM DeliveryTable WHERE EndService IS NULL"
    ...
    Private Sub DGVEmpReport_DoubleClick(sender As Object, e As EventArgs) Handles DGVReport.DoubleClick
        If CheckDGVError(DGVReport) Then Exit Sub
        Dim CR As Integer
        CR = DGVReport.CurrentRow.Index
        DeliveryReportID = DGVReport.Item(0, CR).Value
        FrmDeliveryContainr.LblInfo_Click(sender, e)
    End Sub
