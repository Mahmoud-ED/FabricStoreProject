...
Public Class FrmEmployeeCashing
    Private DS, DsSave, DsSearch As DataSet
    Public CashingID As Integer
    Private TreasuryValue As Double
    Private Show As Boolean ' خاص بعمليات الطرح والجمع في تكست القيمة ف الخزينة
    ...
    Private Sub FrmEmployeeCashing_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged
        DGVCashing.RowTemplate.Height = DGVCashing.Height - DGVCashing.ColumnHeadersHeight
        If Me.Visible Then
    ...
    Private Sub GetData()
        ...
        SQLQuery = " DECLARE @Num Bigint Select @Num=MAX(Num) +1 From EmployeeCashingTable  
        If @Num IS NULL Set @Num= 1  Select @Num Num"
        SQLQuery &= " Select Name FROM EmployeeCashingFinanceTable "
    ...
    Private Sub TxtValue_TextChanged(sender As Object, e As EventArgs) Handles TxtValue.TextChanged
        ...
        If Val(TxtValue.Text) > 0 Then
            TxtNoteFor.Text = "سحب من الخزينة قيمة  / " & Val(TxtValue.Text)
    ...
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles BtnSave.Click
        ...
        If CmbPaymentType.SelectedValue = 1 Then
            If Val(TxtValue.Text) > Val(LblTreasuryValue.Text) Then
                MsgTool("القيمة في الخزينة نقدي لا تساوي القيمة المطلوبة", 0)
                TxtValue.SelectAll()
                TxtValue.Focus()
                Exit Sub
            End If
        ...
        ElseIf CmbPaymentType.SelectedValue > 1 Then
            If Val(TxtValue.Text) > Val(LblValueCard.Text) Then
                MsgTool("القيمة في الخزينة طرق الدفع لا تساوي القيمة المطلوبة", 0)
                TxtValue.SelectAll()
                TxtValue.Focus()
                Exit Sub
            End If
        ...
        Dim param() As SqlParameter = {
            New SqlParameter("CashingID", CashingID),
        ...
        DsSave = Sqlcon.CMDExecuteData("SaveEmployeeCashing", param)
    ...
    Private Sub FillData(DT As DataTable)
        ...
        If DT.Rows.Count > 0 Then
            ...
            With DT.Rows(0)
                ...
                DGVCashing.Item(6, 0).Value = Format(.Item("Date"), GetDateAndTimeFormat(DTFormat.DF))
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        ...
        If MsgBox("هل تريد إلغاء الإيصال ؟ ", vbYesNo + vbQuestion, "تأكيد الإلغاء") = MsgBoxResult.Yes Then
            ...
            Select Case SQLCon.CMDExecute("DeleteEmployeeCashing", 1, Param)
                Case 1
    ...
    '    SQLQuery = "SELECT FORMAT(Num,'000000') AS Num,FORMAT(Date,'" & GetDateAndTimeFormat(DTFormat.DF) & "') AS [Date],Value,EmployeeName,NoteFor,CheckNum,PaymentTypeName FROM EmployeeCashingView WHERE ID=" & CashingID & " AND EndService IS NULL"
    ...
    '    Dim C As New CRTreasuryCashing
    ...
    Public Sub SearchCashing()
        SQLQuery = "SELECT * FROM EmployeeCashingView WHERE  ID= " & CashingID
            ...
            If DsSearch.Tables(0).Rows.Count > 0 Then
                ...
                With DsSearch.Tables(0).Rows(0)
                    ...
                    If Not IsDBNull(.Item("BankID")) Then
                        ...
                        TLPCheck.Enabled = True '   CmbBank.Enabled = True

                    Else
                        CmbBank.SelectedIndex = -1
                        TLPCheck.Enabled = False '    CmbBank.Enabled = False
                    End If
