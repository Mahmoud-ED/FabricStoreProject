Public Class FrmTailorReport
    ...
    Private Sub FrmTailorReport_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged
    ...
    Private Sub Search()
        ...
        SQLQuery = "   Select ROW_NUMBER() OVER (ORDER BY (ID)) As ت, * From TailorsTable where  EndService IS NULL  "

        AppendConditions()

        SQLQuery &= " ORDER BY ID  OFFSET " & PageSize * (PageNum - 1) & " ROWS FETCH NEXT " & PageSize & " ROWS ONLY"

        SQLQuery &= " Select COUNT(TailorsTable.ID)  From  TailorsTable where  EndService IS NULL "
        AppendConditions()
    ...
    Private Sub DGVEmpReport_DoubleClick(sender As Object, e As EventArgs) Handles DGVReport.DoubleClick
        ...
        TailorReportID = DGVReport.Item(0, CR).Value
        FrmTailorContainr.LblInfo_Click(sender, e)
    End Sub
