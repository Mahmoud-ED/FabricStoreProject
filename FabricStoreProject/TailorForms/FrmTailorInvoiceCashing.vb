...
Public Class FrmTailorInvoiceCashing
    Private DS, DsSave As DataSet
    Private CashingID As Integer

    Private Sub FrmTailorInvoiceCashing_VisibleChanged(sender As Object, e As EventArgs) Handles Me.VisibleChanged
        If Me.Visible Then
    ...
    Private Sub FrmTailorInvoiceCashing_Leave(sender As Object, e As EventArgs) Handles Me.Leave
        Clear(TableLayoutPanel10)
        Clear(TableLayoutPanel4)
        Clear(TableLayoutPanel5)
    End Sub
    ...
    Private Sub GetData()
        ...
        SQLQuery = " Select TailorsTable.ID ID ,TailorsTable.Name , COALESCE(SUM(TailorContentTable.Total),0) Total 
                    From TailorsTable, TailorContentTable Where TailorContentTable.TailorID= TailorsTable.ID
                    And TailorContentTable.EndService IS NULL
                    And TailorsTable.ID In(Select TailorID from TailorContentTable) 
                    GROUP BY TailorsTable.ID,TailorsTable.Name ORDER BY TailorsTable.Name   "
        If Not IsNothing(CmbTailor.SelectedValue) Then
            SQLQuery &= " where TailorID=" & CmbTailor.SelectedValue
        End If
        SQLQuery &= " SELECT ID, Name FROM BankTable ORDER BY Name "
        SQLQuery &= " Select TailorsTable.ID, TailorsTable.Name, COALESCE(SUM(CashingTable.Value),0) Value
                    From TailorsTable LEFT JOIN TailorCashingTable ON TailorsTable.ID= TailorCashingTable.TailorID 
                    LEFT JOIN  CashingTable ON CashingTable.ID = TailorCashingTable.CashingID
                    Where CashingTable.EndService IS NULL
                    GROUP BY TailorsTable.ID, TailorsTable.Name "

        If Not IsNothing(CmbTailor.SelectedValue) Then
            SQLQuery &= " And dbo.TailorContentTable.TailorID = " & CmbTailor.SelectedValue
        End If
    ...
    Private Sub Fillcmb()
        ...
        CmbTailor.DataSource = DS.Tables(0)
    ...
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles BtnSave.Click
        ...
        DsSave = Sqlcon.CMDExecuteData("SaveTailorCashing", param)
    ...
    Private Sub CmbPaymentType_KeyPress(sender As Object, e As KeyPressEventArgs) Handles CmbPaymentType.KeyPress, CmbTailor.KeyPress
    ...
    Private Sub TxtValue_TextChanged(sender As Object, e As EventArgs) Handles TxtValue.TextChanged
        ...
        If TxtValue.Text <> "" Then
            If Val(TxtValue.Text) = (Val(TxtSpusedValue.Text) - Val(TxtValueRecipted.Text)) Then
                TxtNoteFor.Text = "دفعة نهائية لحساب الخياط (" & CmbTailor.Text & ") "
            Else
                TxtNoteFor.Text = "دفعة على الحساب " & " للخياط ( " & CmbTailor.Text & " ) , الباقي " & Val(TxtSpusedValue.Text) - (Val(TxtValueRecipted.Text) + Val(TxtValue.Text))
            End If
        End If
    ...
    Private Sub CmbTailor_TextChanged(sender As Object, e As EventArgs) Handles CmbTailor.TextChanged
        If sender.BackColor <> SystemColors.Window Then sender.BackColor = SystemColors.Window
        If IsNothing(CmbTailor.SelectedValue) Then
            Clean()
            Exit Sub
        End If
        TxtRecipient.Text = CmbTailor.Text
    ...
    Private Sub CmbSupplier_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbTailor.SelectedIndexChanged
